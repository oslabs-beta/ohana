...
spacesController.addNamespace = (req, res, next) => {
  ...
  const query =
    "INSERT INTO namespaces (name, cluster_id, team_id) VALUES ($1, $2, $3)";
...
spacesController.createNamespace = (req, res, next) => {
  ...
  runTerminalCommand(gcloud.getCredentials(hostCluster)).then((data) => {
    console.log(data);
    runTerminalCommand(kubectl.createNamespace(hostNamespace)).then((data) => {
      console.log(data);
      return next();
    });
  });
...
spacesController.deploy = (req, res, next) => {
  ...
  runTerminalCommand(
    kubectl.deployImage(deploymentName, deployHostNamespace, imageFile)
  ).then(() => {
    runTerminalCommand(kubectl.expose(deploymentName, deployHostNamespace))
      .then(() =>
        runTerminalCommand(
          `kubectl get services -n ${deployHostNamespace} ${deploymentName}`
        )
      )
      .then((data) => {
        console.log(data);
...
spacesController.getExternalIp = (req, res, next) => {
  ...
  runTerminalCommand(
    kubectl.exposedIP(deploymentName, deployHostNamespace)
  ).then((data) => {
